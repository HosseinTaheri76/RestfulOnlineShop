...
class ProductCompareSerializer(serializers.Serializer):
    ...
    def get_comparison_data(self):
        ...
        products = queries.get_products(prefetch_attribute_values=True).filter(id__in=product_ids)
        ...
        for product in products:
            ...
            for sku in product.prefetched_skus:
                for pav in sku.prefetched_attribute_values:
                    if not pav.value:
                        continue
                    attr_title = pav.value.product_attribute.title
                    attr_value = pav.value.value
